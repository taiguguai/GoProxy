...
- [🔗 API 获取代理](#-api-获取代理) - 接口地址、单选/多选筛选参数
...
- Docker Engine `24+` 与 Docker Compose Plugin（推荐，容器部署）
- 或 Go `1.25+` + CGO 编译环境（本地运行，依赖 `github.com/mattn/go-sqlite3`）
...
```bash
# 1) 安装 Go 与 CGO 相关编译组件
sudo apt-get update
sudo apt-get install -y golang gcc libc6-dev make
...
> 如果系统仓库里的 Go 版本低于 `1.25`，请从 Go 官网安装更新版本。
...
通过 WebUI 的只读接口获取可用代理列表：

- 接口：`GET /api/proxies`
- 示例：`http://your-server-ip:7778/api/proxies`

### 筛选参数

- `protocol`：协议，支持 `http`、`socks5`
- `country`：国家代码（两位，如 `US`、`JP`、`SG`）
- `quality`：质量等级，支持 `S`、`A`、`B`、`C`

说明：
- 同一字段支持多选（OR 关系）
- 不同字段之间是 AND 关系
- 参数大小写不敏感（如 `HTTP`、`sOcKs5` 也可）

### 单选示例

```bash
curl "http://your-server-ip:7778/api/proxies?protocol=http&country=US&quality=A"
```
...
```bash
curl "http://your-server-ip:7778/api/proxies?protocol=http,socks5&country=US,JP,SG&quality=S,A"
```

重复参数：

```bash
curl "http://your-server-ip:7778/api/proxies?protocol=http&protocol=socks5&country=US&country=JP&quality=S&quality=A"
```
